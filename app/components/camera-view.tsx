...
const CameraView: React.FC<CameraViewProps> = ({ onImageCapture }) => {
    ...
    const loadOpenCV = async () => {
      if (typeof window !== 'undefined' && !opencvRef.current) {
        try {
          ...
          await new Promise<void>((resolve, reject) => {
            ...
            script.onload = () => {
              ...
              if (window.cv) {
                ...
                reject(new Error('OpenCV not found after loading'))
              }
            }
          ...
          setDetectionMessage('Detection features unavailable')
  ...
  useEffect(() => {
    if (isDetectionActive && opencvRef.current) {
      ...
      setDetectionMessage(isDetectionActive ? 'Detection starting...' : 'Detection paused')
    }
    ...
    try {
      ...
      if (imageSrc) {
        ...
        if (detectedRectangles.length > 0) {
          finalImageSrc = await cropImageToRectangle(imageSrc, detectedRectangles[0])
